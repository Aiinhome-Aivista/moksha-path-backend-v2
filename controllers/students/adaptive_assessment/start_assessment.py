...
def addaptive_start_assessment():
    conn = None
    try:
        # 1. AUTHENTICATION
        ...
        if not data or 'assignment_id' not in data:
            return api_response(message="assignment_id is required in the request body", code=400, status="error")
        ...
        query = """
            CALL learning.usp_start_student_assessment_v1(
                %s::INTEGER, %s::INTEGER, 
                NULL, NULL, NULL
            )
        """
