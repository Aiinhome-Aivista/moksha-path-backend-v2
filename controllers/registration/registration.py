...
#         cur.execute(
#             "CALL login.usp_v4_send_otp(%s, %s, %s, NULL::VARCHAR, NULL::VARCHAR, NULL::INTEGER, NULL::BOOLEAN)", 
...
#         cur.execute(
#             """CALL login.usp_v4_verify_otp_and_login(
#                 %s, %s, %s, %s, %s, %s, %s, 
...
#             exp_str = exp_time.strftime('%a, %d %b %Y %H:%M:%S GMT') # Formatted GMT string

#             if session_id:
#                 # SCENARIO A: 1 Complete Profile (Fully Logged In)
#                 roles = profiles[0].get('roles') or []
#                 active_sub_id = roles[0].get('subscription_id') if roles else None

#                 # 1. JWT Payload (Requires integer for 'exp')
#                 jwt_payload = {
#                     "sub": str(profiles[0].get('sub')),
...
#                 # 2. JSON Response Payload (Requires string for 'exp')
...
#                 %s::bigint, %s, %s, %s, %s, 
#                 ''::varchar, ''::varchar, '{}'::jsonb, 0::integer, ''::varchar, 0::integer
#             )""", 
#             (user_id, subscription_id, ip_address, user_agent, refresh_token)
#         )
#         result = cur.fetchone()
...
#             # 1. JWT Payload (Requires integer)
#             jwt_payload = {
#                 "sub": str(user_id),
#                 "name": full_name,
#                 "username": username,
...
#             # 2. JSON Response Payload (Requires string)
#             user_response = jwt_payload.copy()
#             user_response["exp"] = exp_str
...
#     User clicks a specific profile. Creates session, token, and refresh token.
#     Body: { "user_id": 305, "subscription_id": "..." }
#     """
#     conn = None
#     try:
#         data = request.get_json()
...
#         # Generate entirely new refresh token for this specific session
#         random_str = str(uuid.uuid4())
#         refresh_token = hashlib.md5(random_str.encode()).hexdigest()
...
#             # Fetch explicitly returned subscription from roles or fallback
#             active_sub_id = roles[0].get('subscription_id') if roles else subscription_id

#             exp_time = datetime.datetime.utcnow() + datetime.timedelta(hours=12)
#             exp_str = exp_time.strftime('%a, %d %b %Y %H:%M:%S GMT') # Formatted GMT string

#             # 1. JWT Payload (Requires integer timestamp)
#             jwt_payload = {
#                 "sub": str(user_id),
#                 "name": full_name,
#                 "username": username,
...
#     Body: { "user_id": 356, "subscription_id": "...", "force_login": true/false }
#     """
#     conn = None
#     try:
#         data = request.get_json()
#         user_id = data.get('user_id')
#         subscription_id = data.get('subscription_id') 
#         force_login = data.get('force_login', False) # Defaults to False
...
#         # Note the two extra ''::varchar added here for email and phone outputs
#         cur.execute(
#             """CALL login.usp_v4_select_profile(
#                 %s::bigint, %s, %s, %s, %s, %s::boolean,
#                 ''::varchar, ''::varchar, ''::varchar, ''::varchar, '{}'::jsonb, 0::integer, ''::varchar, 0::integer
#             )""", 
#             (user_id, subscription_id, ip_address, user_agent, refresh_token, force_login)
#         )
#         result = cur.fetchone()
...
#             full_name = result['o_full_name']
#             username = result['o_username']
#             email = result['o_email']       # <-- NEW
#             mobile = result['o_phone']      # <-- NEW
#             roles = result['o_role_json']
...
#             # Build Exact User Payload including email and mobile
#             jwt_payload = {
#                 "sub": str(user_id),
#                 "email": email,             # <-- NEW
#                 "mobile": mobile,           # <-- NEW
...
#         # =======================================================
#         # NEW: Catch "Already Logged In" and prompt frontend
#         # =======================================================
#         elif result['o_status_code'] == 409:
#             # We return 200 Success so the UI doesn't crash, but pass a flag indicating force is needed
#             return api_response(
