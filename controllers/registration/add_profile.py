...
def add_profile_v4():
    ...
    try:
        ...
        if result['o_status_code'] == 200:
            ...
            # =========================================================
            # STRIP SUBSCRIPTION ID FROM ROLES (For Main Auth Token)
            # =========================================================
            cleaned_roles = [{k: v for k, v in r.items() if k != 'subscription_id'} for r in roles]
            ...
            sub_payload = {
                "sub": str(user_id),
                "role_id": role_id_value,
                "roles": roles, # Kept original roles array with subscription_id here
                "sub_id": active_sub_id,
                "exp": int(exp_time.timestamp())
            }
            sub_token = jwt.encode(sub_payload, JWT_SECRET, algorithm="HS256")
            ...
            final_response = {
                "auth_token": auth_token,          # Replaced single token with auth_token
                "refresh_token": refresh_token,
                "subscription_id": active_sub_id,
                "subscription_token": sub_token,   # Generated dedicated sub token
                "user": user_response
            }
            return api_response(message=result['o_message'], code=200, data=final_response, status="success")
