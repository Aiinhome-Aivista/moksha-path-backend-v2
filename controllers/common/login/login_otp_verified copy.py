...
def login_login_user():
    ...
    try:
        ...
        conn = get_db_connection()
        conn.autocommit = True
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) # Ensure DictCursor is used
        ...
        # 2. Call Procedure: usp_v1_login_user
        # FIX: Added one extra NULL::VARCHAR at position 11 for 'p_subscription_id'
        cur.execute("""
            CALL login.usp_v2_login_user(
                %s, %s, %s, %s, %s, %s, 
                NULL::INTEGER,   -- p_user_id
                NULL::VARCHAR,   -- p_full_name_out
                NULL::JSONB,     -- p_role_json
                NULL::INTEGER,   -- p_session_id
                NULL::VARCHAR,   -- p_subscription_id (NEW ADDITION)
        ...
        if result['p_status_code'] == 200:
            ...
            payload = {
                ...
                "roles": result['p_role_json'],
                "sid": str(result['p_session_id']),
                "sub_id": result['p_subscription_id'], # Add Subscription ID to Token
                "exp": datetime.datetime.utcnow() + datetime.timedelta(hours=12)
            }
            token = jwt.encode(payload, JWT_SECRET, algorithm="HS256")
            ...
            return api_response(message=result['p_message'], code=200, data={
                ...
                "user": payload,
                "subscription_id": result['p_subscription_id'] # Return in API response too
            })
